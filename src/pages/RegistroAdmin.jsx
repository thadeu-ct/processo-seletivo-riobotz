...
function RegistroAdmin() {
  ...
  const handleToggleRegistro = (matricula, statusAtual) => {
    setUsuarios((prev) =>
      prev.map((u) =>
        u.matricula === matricula ? { ...u, registrado: !statusAtual } : u,
      ),
    );
  };
      ...
      <div className="w-11/12 max-w-5xl flex flex-col gap-6">
        <div className="bg-white rounded-lg p-6 shadow-xl overflow-hidden">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            <h2 className="text-[#0a1945] font-extrabold text-xl">
            ...
            <div className="flex items-center gap-4">
              {msgSalvar.texto && (
                <span
                  className={`text-sm font-bold ${msgSalvar.tipo === "sucesso" ? "text-green-600" : "text-red-600"}`}
                >
                  {msgSalvar.texto}
                </span>
              )}
