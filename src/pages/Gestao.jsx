...
function Gestao() {
  ...
  const [trilhaGestao, setTrilhaGestao] = useState([]);
  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        ...
        console.error("Erro ao carregar trilha Gestão:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      {loading ? (
        <main className="flex-grow flex flex-col items-center justify-center bg-[#0a1945]">
          <div className="relative flex items-center justify-center w-32 h-32">
            <svg
              className="w-full h-full text-cyan-400 anim-dash opacity-80 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="1"
          ...
          <h2 className="text-cyan-400 font-black uppercase tracking-[0.3em] text-xs mt-8 animate-pulse">
        ...
        <main className="flex-grow flex flex-col items-center py-8 px-4 md:px-8 relative">
          ...
          <div className="mb-12 w-full max-w-4xl flex items-center gap-4 md:gap-6">
            ...
            <div className="flex flex-col">
              <h1 className="text-cyan-400 font-black text-4xl md:text-5xl lg:text-6xl tracking-tight uppercase font-mono m-0">
                Gestão
              </h1>
              {finalAdminView && (
          ...
          <div className="w-full max-w-4xl relative">
            <div className="absolute left-[19px] md:left-[39px] top-4 bottom-0 w-1 bg-cyan-400/20 rounded-full"></div>
            <div className="flex flex-col gap-12">
              {trilhaGestao.map((item, index) => (
