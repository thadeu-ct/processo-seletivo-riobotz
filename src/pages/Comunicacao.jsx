...
function Comunicacao() {
  ...
  const [trilhaComunicacao, setTrilhaComunicacao] = useState([]);
    ...
    const carregarDados = async () => {
      try {
        ...
        const resW = await fetch(`${API_URL}/workshops/area`, {
          ...
          body: JSON.stringify({ area: "comunicacao" }),
        });
      ...
      {loading ? (
        <main className="flex-grow flex flex-col items-center justify-center bg-[#0a1945]">
          <div className="relative flex items-center justify-center w-32 h-32">
            <svg
              className="w-full h-full text-fuchsia-400 anim-dash opacity-80 drop-shadow-[0_0_15px_rgba(232,121,249,0.5)]"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="1"
            ...
            <div className="absolute inset-0 border-4 border-fuchsia-400/20 border-t-fuchsia-400 rounded-full animate-spin"></div>
          </div>
          <h2 className="text-fuchsia-400 font-black uppercase tracking-[0.3em] text-xs mt-8 animate-pulse">
            Sincronizando Comunicação...
          </h2>
        </main>
      ) : (
