...
function Eletronica() {
  ...
  const [trilhaEletronica, setTrilhaEletronica] = useState([]);
    ...
    const carregarDados = async () => {
      try {
        ...
        const resW = await fetch(`${API_URL}/workshops/area`, {
          ...
          body: JSON.stringify({ area: "eletronica" }),
        });
        ...
        if (Array.isArray(workshops)) {
          ...
          setTrilhaEletronica(ordenados);
        }
      } catch (err) {
        console.error("Erro ao carregar trilha Eletrônica:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      {loading ? (
        <main className="flex-grow flex flex-col items-center justify-center bg-[#0a1945]">
          <div className="relative flex items-center justify-center w-32 h-32">
            <svg
              className="w-full h-full text-yellow-400 anim-dash opacity-80 drop-shadow-[0_0_15px_rgba(250,204,21,0.5)]"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="1"
