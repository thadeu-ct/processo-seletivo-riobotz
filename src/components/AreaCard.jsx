...
function AreaCard({
  ...
  const cardContent = (
    ...
    >
      <div
        className={`absolute -right-4 -bottom-4 opacity-10 ${textClass} group-hover:scale-110 transition-transform duration-500`}
      >
        <svg
          className="w-40 h-40"
          fill="none"
      ...
      <div className="mb-4 relative z-10">
        <h2
          className={`font-black text-3xl tracking-tighter uppercase ${textClass}`}
        >
          {titulo}
        </h2>
      </div>
      ...
      {isLocked && (
        <div className="absolute inset-0 bg-[#0a1945]/90 backdrop-blur-sm rounded-3xl flex flex-col items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
          ...
          <span className="text-amber-400 font-black uppercase text-xl">
            Bloqueado
          </span>
